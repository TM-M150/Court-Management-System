<?php require_once 'db_con.php'; 
	...
	if (isset($_POST['register'])) {
		...
		if (empty($name)) {
			...
			$input_error['national_id'] = "The id Field is Required";
		}if (empty($phone)) {
			$input_error['phone'] = "The phone Field is Required";
		}
		if (empty($email)) {
				...
				if (mysqli_num_rows($check_username)==0) {
					if (strlen($username)>7) {
						if (strlen($password)>7) {
								$password = $password;
							$query = "INSERT INTO `users`(`name`, `email`, `username`, `password`, `photo`, `status`, `national_id`, `phone`) VALUES ('$name', '$email', '$username', '$password','$photo_name','active','$national_id','$phone');";
...
<html lang="en">
  ...
  <body>
    <div class="container"><br>
          <h1 class="text-center">Create Account With Us</h1><hr><br>
            ...
            <div class="col-md-8 offset-md-2">
             	<form method="POST" enctype="multipart/form-data">
				  <div class="form-group row">
					...
					<div class="col-sm-4">
				      <input type="text" class="form-control" value="<?= isset($national_id)? $national_id:'' ?>" name="national_id" placeholder="national_id/Business Number" id="inputEmail3"><?= isset($input_error['national_id'])? '<label for="inputEmail3" class="error">'.$input_error['national_id'].'</label>':'';  ?>
				    </div>
				  ...
				  <div class="col-sm-4">
				      <input type="text" class="form-control" value="<?= isset($phone)? $phone:'' ?>" name="phone" placeholder="phone" id="inputEmail3"><?= isset($input_error['phone'])? '<label for="inputEmail3" class="error">'.$input_error['phone'].'</label>':'';  ?>
				    </div></br>
				  <div class="row">
				  	<div class="col-sm-3"><label for="photo">Profile Picture</label></div>
				  	<div class="col-sm-9">
