...
<table class="table  table-striped table-hover table-bordered" id="data">
  ...
  <tbody>
   <?php $showuser = $_SESSION['user_login']; $haha = mysqli_query($db_con,"SELECT * FROM `users` WHERE `username`='$showuser';"); $showrow=mysqli_fetch_array($haha); ?>
    <?php 
	$id=$showrow['national_id'];
      $query=mysqli_query($db_con,"SELECT * FROM `casefile` WHERE `national_id`= $id ORDER BY `casefile`.`datetime` DESC");
      $i=1;
...
<a class="btn btn-xs btn-success" href="index1.php?page=view&id='.base64_encode($result['id']).'&caseno='.base64_encode($result['caseno']).'">
